...
*blockcell* 内置一层状态机专用图元——`state-chain`。定位：*线性状态链 +
若干自环 + 若干越级跳转*，覆盖业务上绝大多数 "这件事有几个阶段，相邻之间可转
移、偶尔有跳跃" 的需求（文件 I/O、订单生命周期、连接状态机……）。

非线性拓扑（N×N 任意互跳、任意出度入度）仍然建议跳 `fletcher` / `cetz`——
本章的 API 用 *id 引用 + 语义路由* 替代坐标，代价就是只能表达 "沿链前进 + 对
已命名状态做跨越"，不能画真正的 2D 图。
...
#align(center)[
  #region(fill: rgb("#FCE4EC"), width: 490pt)[
    ...
    #grid(
      ...
      text(size: 0.85em)[`state(id)` —— 自动圆形，按文字宽度自适应；`initial: true` / `accept: true` 着色 + 双边框],
      text(size: 0.85em, weight: "bold")[链上转移],
      text(size: 0.85em)[目标节点的 `edge-label:` 给相邻箭头加标（同 `flow-col` 规则）],
      text(size: 0.85em, weight: "bold")[自环],
      text(size: 0.85em)[`loop(id)[label]`——状态上方（默认）或下方的小弧],
      text(size: 0.85em, weight: "bold")[越级转移],
      text(size: 0.85em)[`jump(from, to)[label]`——两个状态之间的大弧，`route: "above" / "below"`],
...
```typst
#state("id", initial: false, accept: false, edge-label: none,
...
#grid(
  ...
  text(weight: "bold")[`initial`], [设为 `true` → 默认绿底，链首自动画一个灰色入口小圆点 + 进入箭头],
  text(weight: "bold")[`accept`], [设为 `true` → 默认黄底 + 双边框（UML 的"接受状态"）],
  text(weight: "bold")[`edge-label`], [链上 *进入* 本状态的箭头标签],
...
在指定 id 的状态上画一段回到自身的小弧，带标签。`route: "above"` / `"below"`
控制方向（默认 above），`style: "dashed"` 画虚线弧（表示可选/条件转移）。
...
从 `from` 状态到 `to` 状态画一条大弧（三次贝塞尔曲线），`route` 决定走链上方
还是下方。方向可以是正向（from → 右侧的 to）或反向（from → 左侧的 to，表达
"回退"），算法对两个方向对称。
...
#grid(
  columns: (110pt, 1fr),
  row-gutter: 4pt,
  text(weight: "bold")[`gap`], [相邻状态圆的水平间距，默认 60pt],
  text(weight: "bold")[`loop-height`], [自环弧的最高点相对状态上/下边缘的距离，默认 28pt],
  text(weight: "bold")[`jump-height`], [越级弧的最高点相对链上/下边缘的距离，默认 48pt],
  text(weight: "bold")[`min-size`], [状态最小直径，默认 44pt],
)

== 局限

- *只是一条链*——不能画 N×N 任意拓扑。多条 `jump` 会简单按 above/below 堆叠，
  不做避让。
- *没有复合状态*——UML 的 "state with substates" 要自己用嵌套 region + state-chain
  模拟。
- *不做自动编号/转移表*——如果要生成文档索引，用 Typst 的 `locate` 配合自定义
  metadata，本章不涉及。

真正需要非线性状态图时，`fletcher` 允许任意 `(x, y)` 节点和任意弯曲的边——代价
是要手动摆坐标，5 个以上状态就比 `state-chain` 啰嗦不少。
