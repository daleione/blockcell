...
- *线性模式*下，`route: "above" / "below"` 决定弧往链的哪一边走；算法对正向
  （from 在左）和反向（from 在右）两种写法对称。`height:` 可以单独指定这
  条弧的峰深（默认取 `state-chain` 的 `jump-height`）——多条同侧 jump 嵌
  套时，给短的那条传更小的 `height:` 就能让它收在长弧内侧。
...
- 所有状态圆按最长的 `ESTABLISHED` 统一大小——`state-chain` 自动把所有 `size:
  auto` 的状态对齐到池里最大的自然直径，不会出现不规则大小。
- LISTEN 同时挂着 `loop`（自环）和 `listen→closed`（跳转），两者默认都走
  "上方"也不会撞：`loop` 锚在状态正上方（±6pt），`jump` 锚自动挪到朝向
  目标那一侧的 *45° 位置*，两者占据状态边上的不同扇区。
- 一条 `above` 一条 `below`，避免两条 `close()` 的长弧在同一侧堆叠。如果
  确实需要多条同侧 jump 嵌套，给短的那条传 `height:` 让它收浅，比长的那
  条至少小一半，两条不会相交。
