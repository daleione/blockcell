...
- **Timeline (`timeline`)**: proportional time track / single-track Gantt. Segment widths encode durations via date mode (`to:` + `start:`) or weight mode (`span:`, number or `duration`); date ticks are pinned to the boundaries automatically. Supports `inside`/`above`/`below` label placement, palette-cycled fills, and per-segment style overrides. Shares a new internal proportional-layout core (`_span-cols`) with `bit-row` (refactored onto it; public API unchanged). Design notes in `docs/research/timeline-composite-design.md`.
- **`cell` `align:` override**: optional explicit content alignment; `auto` (default) keeps the existing subtitle-based heuristic, so fixed-height cells (e.g. `timeline` segments) can vertically center a single label.
